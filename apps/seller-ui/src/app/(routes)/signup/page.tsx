...
const Signup = () => {
  ...
  const connectCashfree = async () => {
    if (!sellerId) {
      alert("Seller ID not found. Please complete previous steps first.");
      return;
    }

    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/api/create-cashfree-vendor`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        ...
        alert(
          data?.message ||
            "❌ Failed to create Cashfree vendor. Please try again."
        );
        return;
      }

      console.log("Cashfree vendor:", data.vendor);
      alert("✅ Cashfree vendor created successfully!");
    } catch (error) {
      console.error("Error creating Cashfree vendor:", error);
      alert("❌ Failed to create Cashfree vendor. Please try again.");
    }
  };
      ...
      <div className="md:w-[480px] p-8 bg-white shadow rounded-lg">
        ...
        {activeStep === 3 && (
          <div className="text-center">
            <h3 className="text-2xl font-semibold mb-4">Connect Cashfree</h3>
            <button
